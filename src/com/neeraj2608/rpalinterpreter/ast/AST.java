...
public class AST{
  ...
  private void standardize(ASTNode node){
    ...
    switch(node.getType()){
      ...
      case REC:
        ...
        break;
      case LAMBDA:
        // This part causes the standardized tree to look different from that generated by Walstra's implementation!!
        // I choose to standardize the LAMBDA nodes because the CSE rules do not explicitly say not to and because
        // the standardized tree should be as close to a binary tree as possible.
        //     LAMBDA        LAMBDA
        //      /   \   ->   /    \
        //     V++   E      V     .E
  ...
  private ASTNode constructLambdaChain(ASTNode node){
    ...
    if(node.getSibling()!=null && node.getSibling().getSibling()!=null)
